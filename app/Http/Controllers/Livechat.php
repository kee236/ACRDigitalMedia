...
class Livechat extends Home
{
    ...
    {
        $this->set_global_userdata(true,['Admin','Member'],[],$this->ai_livechat);
    ...
    public function load_livechat()
    {
        ...
        $data['body'] = 'openai.livechat-default';
        $custom_prompts =DB::table('ai_chat_settings')->select('id','profile_name','custom_prompt','chat_model','custom_prompt_img')->limit(20)->get();                             
        $data['custom_prompts']= isset($custom_prompts) ? $custom_prompts : '';
        return $this->viewcontroller($data);       

    }
    public function load_livechat_default()
    {
        if($this->is_member && !$this->is_manager && config('app.force_email_verify')=='1' && !auth()->user()->email_verified_at){
            return redirect(route('verification.notice'));
        }
    ...
    public function livechat_conversation(){ 
        ...
        $sendCounter = request()->sendCounter ?? 0;
        $system_prompt_value='';
        $system_prompt_model='';
       ...
       $content='<li class="replies">
                <img src="'.$ai_img.'" alt="" />
                <p class="lh-base conversation_block_'.$conversation_id.'_'.$sendCounter.'" id="reply_'.time().'0">'.$this->convert_code_block($answer_by_AI,$conversation_id.'_'.$sendCounter,true).'</p>
                </li>';
    ...
    public function convert_code_block($text,$conversation_id="",$addClass=false) {
        $pattern = '/```(.+?)```/s'; // pattern to match the code block enclosed in triple backticks
        $has_code=0;

        $converted_text = preg_replace_callback($pattern, function($matches) use (&$has_code,$addClass,$conversation_id) {
        $has_code=1;
        $code = $matches[1];
        $code = htmlspecialchars($code); // convert special characters to HTML entities
        $code = nl2br($code); // replace newlines with <br> tags
        $class = $addClass ? "conversation_block_".$conversation_id : "";
        $id = $addClass ? "id='reply_".time().rand()."1'" : "";
        $embedded_code = "</p></li><pre><code class='".$class."' ".$id." >" . $code . "</code></pre><li class='replies'><p class='".$class."' id='reply_".time().rand()."2'>";
